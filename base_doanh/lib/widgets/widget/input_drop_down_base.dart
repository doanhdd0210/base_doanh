...
import 'package:gen_crm/src/extensions/common_ext.dart';
...
class _InputDropdownState extends State<InputDropdownBase> {
  final BehaviorSubject<List<dynamic>> _selectStream =
      BehaviorSubject.seeded(['', '']);
  final BehaviorSubject<List<List<dynamic>>?> _listSelectStream = BehaviorSubject();
  dynamic _idDF = '';
    ...
    try {
      ...
      if (response.statusCode == 200) {
        if (response.data['data'] != null) {
          ...
          if (res.isNotEmpty && fieldSetValueDatasource.isNotEmpty) {
            res.forEach((element) {
              if (fieldSetValueDatasource.first.isNotEmpty) {
          ...
          if (widget.data.field_name == 'mau_sac') {
            //case dac biet
            if (res.isNotEmpty) _selectStream.add(res.first);
  ...
  void _onChangeMain(id, bool? isCK) {
    closeKey();
  ...
  void _onDropDown(List<dynamic> v) async {
...
class _DropDownSearchApiState extends State<DropDownSearchApi> {
  final LoadMoreController _loadMoreController = LoadMoreController();
  String _txtSearch = '';
  List<List<dynamic>> _listAdd = [];

  Future<dynamic> getList({
    String? txt,
    int page = BaseURL.PAGE_DEFAULT,
  }) async {
    try {
      var dio = DioProvider.dio;
      var response = await dio.request(
          ...
          Expanded(
            child: ViewLoadMoreBase(
              ...
              itemWidget: (int index, data) {
                if (_listAdd.isNotEmpty && index == 0) {
                  return Column(
                    children: [
                      ..._listAdd
                          .map(
